...
const ServiceReplacement = () => {
	...
	const [initialValue, setInitialValue] =
		useState<ICreateServiceReplacementRequest>();
	...
	const onFinish = (values: any) => {
		if (showModal.id) {
			updateServiceReplacementApi(showModal.id, values).then(res => {
				if (res.data.status === "success") {
			...
			createNewServiceReplacementApi(values).then(res => {
				if (res.data.status === "success") {
					setShowModal({ show: false });
					fetchDataList();
				}
