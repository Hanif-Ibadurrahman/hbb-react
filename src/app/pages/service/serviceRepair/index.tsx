...
const ServiceRepair = () => {
		...
		swalCustom
			...
			.then(result => {
				if (result.isConfirmed) {
					approveServiceRepairApi(id, { remark: result.value })
						.then(res => {
	...
	const handleReject = (id: number) => {
		...
		swalCustom
			...
			.then(result => {
				if (result.isConfirmed) {
					rejectServiceRepairApi(id, { remark: result.value })
						.then(res => {
							if (res.data.status === "success") {
								swalCustom.fire(
									"Reject",
	...
	const onFinish = (values: any) => {
		...
		if (showModal.id) {
			updateServiceRepairApi(showModal.id, input)
				.then(res => {
					if (res.data.status === "success") {
						...
						<TablePaginateAndSort
							...
							contentHeader={
								...
								>
									...
									{listCheckPermission.isAllowCreateServicePerbaikan && (
										<button
											type="button"
