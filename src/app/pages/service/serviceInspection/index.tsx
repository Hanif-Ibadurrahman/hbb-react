...
import {
	...
	IServiceInspectionPaginateResponse,
} from "store/types/serviceInspectionTypes";
import {
	approveServiceInspectionApi,
	createNewServiceInspectionApi,
	deleteServiceInspectionApi,
	getAllServiceInspectionApi,
	getDetailServiceInspectionApi,
...
const ServiceInspection = () => {
	...
	const fetchDataList = async () => {
		try {
			if (params) {
				const response = await getAllServiceInspectionApi(params);
		...
		swalCustom
			...
			.then(result => {
				if (result.isConfirmed) {
					approveServiceInspectionApi(id).then(res => {
	...
	const handleReject = (id: number) => {
		...
		swalCustom
			...
			.then(result => {
				if (result.isConfirmed) {
					rejectServiceInspectionApi(id, { remark: result.value }).then(res => {
						if (res.data.status === "success") {
							swalCustom.fire(
								"Reject",
	...
	const handleDelete = (id: number) => {
		...
		swalCustom
			...
			.then(result => {
				if (result.isConfirmed) {
					deleteServiceInspectionApi(id).then(res => {
						if (res.data.status === "success") {
	...
	const onFinish = (values: any) => {
		...
		if (showModal.id) {
			updateServiceInspectionApi(showModal.id, input)
				.then(res => {
					if (res.data.status === "success") {
				...
				<div className="row">
					<div className="col-12">
						<TablePaginateAndSort
							title="Permintaan Layanan - Pemeriksaan"
							...
							contentHeader={
								...
								>
									...
									{listCheckPermission.isAllowCreateServicePemeriksaan && (
										<button
											type="button"
