...
const ServiceChange = () => {
	...
	const [initialValue, setInitialValue] =
		useState<Partial<ICreateServiceChangeRequest>>();
		...
		swalCustom
			...
			.then(result => {
				if (result.isConfirmed) {
					approveServiceChangeApi(id, { remark: result.value })
						.then(res => {
	...
	const handleReject = (id: number) => {
		...
		swalCustom
			...
			.then(result => {
				if (result.isConfirmed) {
					rejectServiceChangeApi(id, { remark: result.value })
						.then(res => {
							if (res.data.status === "success") {
								swalCustom.fire(
									"Reject",
	...
	const onFinish = (values: any) => {
		...
		if (showModal.id) {
			updateServiceChangeApi(showModal.id, input)
				.then(res => {
					if (res.data.status === "success") {
			...
			createNewServiceChangeApi(input)
				.then(res => {
					if (res.data.status === "success") {
						setShowModal({ show: false });
						fetchDataList();
